...
const { QDRANT_URL, QDRANT_API_KEY, QDRANT_COLLECTION, OPENAI_API_KEY } =
  process.env;
 ...
 */
async function addDocumentsInternal(client, embeddings, documents) {
...
/**
 * Convenience: create (or recreate) the collection from a set of documents.
 * This is what the indexing script uses in "rebuild" mode.
 */
async function createVectorStoreFromDocuments(documents) {
  const client = getQdrantClient();
  const embeddings = getEmbeddings();

  // Drop collection if it exists, then recreate fresh
  try {
  ...
  await addDocumentsInternal(client, embeddings, documents);

  return makeVectorStoreFacade(client, embeddings);
}
 ...
 */
function makeVectorStoreFacade(client, embeddings) {
  return {
    /**
...
/**
 * Get a handle to an existing "vector store" facade.
 * Assumes the collection already exists (created by the indexer).
 * Used by /api/chat/ask.js.
 */
async function getVectorStore() {
  const { client, embeddings } = await ensureCollection();
  return makeVectorStoreFacade(client, embeddings);
}
